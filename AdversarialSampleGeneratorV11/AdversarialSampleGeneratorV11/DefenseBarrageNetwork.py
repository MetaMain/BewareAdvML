...
class DefenseBarrageNetwork():
    def __init__(self, modelDir, totalTransformNumber, classNum, colorChannelNum):
        self.ModelDir = modelDir
        self.TotalTransformNumber = totalTransformNumber
        self.ClassNum = classNum
        self.ColorChannelNum = colorChannelNum
        self.NumCores = 10 #Number of cores for parallel transformations 
    ...
    def GenerateTransformedDataForTesting(self, xData):
        totalSampleNum = xData.shape[0]
        xTransformed = np.copy(xData)+0.5  #Make a copy of the data so that the original does not get overwritten
        for i in range(0, totalSampleNum):
            currentTransformNumber = self.TotalTransformNumber
            if currentTransformNumber > 0 : #Only do transformations if the number is greater than 0, otherwise use original data
                if self.ColorChannelNum == 3: #Color dataset
                    xTransformed[i] = self.BarrageTransformColor(xTransformed[i], currentTransformNumber)
                else: #Grayscale 
    ...
    #Standard evaluation function with barrage of random transforms applied to the input first
    def evaluate(self, xData, yData):
        sampleNumber = xData.shape[0]
        yPredict = self.predict(xData)
        acc = 0
        for i in range(0, sampleNumber):
            if yPredict[i].argmax(axis=0) == yData[i].argmax(axis=0):
                acc = acc + 1
        acc = acc / sampleNumber
        return acc 

    #Standard predict function with barrage of random transforms applied to the input first
    #The data should ALWAYS be mean shifted so it lies in the range -0.5 to 0.5, the code will correct everything else
    def predict(self, xData):
        sampleNumber = xData.shape[0]
        print("Doing randomized input transformations...")
        print("Number of transformations per image is:", self.TotalTransformNumber)
        xDataTransformed = np.copy(xData)+0.5 #Make a deep copy of the array so the original data doesn't get overwritten
        xDataTransformed= np.clip(xDataTransformed, 0, 1.0) #Sometimes when doing the mixed-black box attack values can fall outside the range so need to fix
        #Also add 0.5 because the image transformations needs to be in the 0-1 range 
        num_cores = self.NumCores
        if sampleNumber % num_cores == 0:
            print("Doing CPU based parallelization for the transformations.")         
            inputs = range(num_cores)
            stepSize = int(sampleNumber/num_cores)
            results = Parallel(n_jobs=num_cores)(delayed(self.ParallelTransform)(xDataTransformed[i*stepSize:(i+1)*stepSize]) for i in inputs)
            indexer =0
            for i in range(0, num_cores): #This maps the data back to xDataTransform
                for j in range(0, stepSize):
                    xDataTransformed[indexer] = results[i][j]
                    indexer = indexer + 1
            #results = Parallel(n_jobs=num_cores)(delayed(self.ParallelTransform)(xDataTransformed[i*stepSize:(i+1)*stepSize]) for i in inputs)
        else:
            print("Sample number indivisable by core number. Doing transformations in series.")
            self.SeriesTransform(xDataTransformed)
        #for i in range(0, sampleNumber):
        ...
        #print("Randomized transformations complete.")
        xDataTransformed = xDataTransformed - 0.5 #The classifier was trained on data in range -0.5 to 0.5 so need to put back in the proper range
        model = DataManager.LoadModel(self.ModelDir)
        yPredict = model.predict(xDataTransformed)
        model = None #Joblib will complain if we do not get rid of the model after use
        return yPredict
    
    #def predictDebug(self, xData):
    #    sampleNumber = xData.shape[0]
    #    print("Doing randomized input transformations...")
    #    print("Number of transformations per image is:", self.TotalTransformNumber)
    #    xDataTransformed = np.copy(xData)+0.5 #Make a deep copy of the array so the original data doesn't get overwritten
    ...
    #        #results = Parallel(n_jobs=num_cores)(delayed(self.ParallelTransform)(xDataTransformed[i*stepSize:(i+1)*stepSize]) for i in inputs)
    #    else:
    #        print("Sample number indivisable by 1000 or too small. Doing transformations in series.")
    #        self.SeriesTransform(xDataTransformed)
    #    return xDataTransformed
    ...
    def Group6Transformation(self, img):
        transformIndex = int(self.randUnifI(0, 1)) #Choose between 0 and 1
        if transformIndex == 0:
            img = self.Group6p1ZoomRandom(img)
            #print("6")
        else:
            img = self.Group6p2SeamCarvingExpansion(img)
            #print("7")
        return img
